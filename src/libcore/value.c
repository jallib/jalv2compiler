...
/* jalv25r9. 
   ...
   function pic_assign_to_ptr() to be able to get the correct offset 
   for a label using a lookup table when using arrays in JAL records. 
   The 'data' itself is generated by the function jal_parse_subscript(). */
    ...
    if (!ptr) {
        ...
        if (!ptr->baseofs) {
            n = 0;
             ...
             /* We need to use the data of the var of baseofs as to obtain the size. 
                Note that variable_const_get() checks if the pointer is valid. */
             n = variable_const_get(ptr->baseofs->var, value_def_get(val), pos);
        }
    }
    return n;
}
